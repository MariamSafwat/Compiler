...
def drawing (output):
    ...
    prev_repeat = None
    while k < len(output):
        if(output[k][1]=='READ'): ##done
            G1.add_edges_from([(output[k][0],output[k+1][0])])
        elif(output[k][1]=='IF'):
            prev_if = k
            G1.add_edges_from([(output[k][0],output[k+2][0]),(output[k][0],output[k+4][0])])
            if(output[k+2][1]=='OP'):
                G1.add_edges_from([(output[k+2][0],output[k+1][0]),(output[k+2][0],output[k+3][0])])
                k = k +4
                continue
            
        elif(output[k][1]=='THEN'):
            if(output[k+2][1]=='ASSIGN'):
            G1.add_edges_from([(output[k][0],output[k+2][0]),(output[k+2][0],output[k+1][0]),(output[k+2][0],output[k+3][0])])
            k = k+3
            continue
        elif(output[k][1]=='ELSE'):
            G1.add_edges_from([(output[prev_if][0],output[k][0]),(output[k][0],output[k+1][0])])
        elif(output[k][1]=='REPEAT'):
            G1.add_edges_from([(output[k][0],output[k+1][0])])
            prev_repeat = k
        elif(output[k][1]=='ID'):
            G1.add_edges_from([(output[k-1][0],output[k][0])])
        elif(output[k][1]=='UNTIL'):
            G1.add_edges_from([(output[prev_repeat][0],output[k][0]),(output[k][0],output[k+1][0])])
            
        elif (output[k][1]=='ASSIGN'):
            if(output[k+1][1]=='CONST'):
                G1.add_edges_from([(output[k][0],output[k+1][0])])
            else :
                G1.add_edges_from([(output[k][0],output[k-1][0]),(output[k][0],output[k+1][0])])
        elif(output[k][1]=='END'):
            G1.add_edges_from([(output[k-1][0],output[k][0])])
        k +=1    
...
def term(token,i):
    i = factor(token,i)
    while mulop(token,i):
        i += 1 ##7atetha 3shan kant bt3ml error nshof akherha fen
        i = factor(token,i)
    return i
